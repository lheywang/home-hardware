---
title: "Refroidissement: airflow et tdp"
description: "La chaleur est régulièrement source de stress, mais d'ou vient-elle ? Pourquoi ?"
cover: "airflow.png"
---
...
Cette valeur a un sens physique : La puissance moyenne que doit être capable d'absorber le refroidissement pour assurer le fonctionnement
...
sa durée de vie (de toute façon trop longue pour en voir le bout en usage réel). Le silicium, quant à lui, ne fond qu'à 1414 degrés, 
aucuns risques donc.
:::
...
<Tabs>
  <TabItem label="Les caloducs">
    ...
    Ces tubes en cuivres, très présents dans un ventirad, GPU ou pc portables,
...
C'est bien beau, maintenant, on sait d'où vient la chaleur, et comment est déplacée. Une dernière problématique apparaît donc :
...
conserver les performances de votre système. Avec suffisamment d'air frais qui passe au travers d'un radiateur, il devient possible
d'en réduire sa taille tout en conservant un refroidissement similaire. Par exemple, dans les serveurs, on refroidit un processeur
consommant beaucoup avec un petit radiateur, mais un débit d'air extrêmement important !

Cependant, votre ventilation pour se montrer efficace doit respecter quelques astuces :

* Pensez à ne pas pousser de l'air chaud dans votre radiateur, cela aura un effet contre-productif

Partant de ce fait, on arrive sur un thème très vague :

## L'airflow.

Ce dernier consiste en la conception d'un boîtier, le placement des ventilateurs et ouvertures de façon à optimiser le refroidissement.
...
<Tabs>
  <TabItem label="Entrées">
    ...
    d'être chauds, donc sur l'avant du boîtier (exposé à l'intérieur de la pièce
    donc).
  ...
  <TabItem label="Sorties">
    ...
    placer sur le haut du boîtier, et sur l'arrière du boîtier (généralement
    placées en haut).
...
Il n'existe aucune manière de quantifier l'airflow : On peut le considérer réussi si les températures sont satisfaisantes, 
et qu'aucun composant ne se limite.

**Est-ce que l'airflow est important ?**

Oui ! C'est même primordial. Un bon airflow compense sans souci des ventilateurs moins performants, mais l'inverse ne sera pas vrai. Prenez plutôt
des ventilateurs qui vous plaisent avec un bon boîtier que l'inverse.

Un bon airflow peut sans souci vous réduire la température de fonctionnement de votre pc, de plusieurs degrés, voire dizaines de degrés !
