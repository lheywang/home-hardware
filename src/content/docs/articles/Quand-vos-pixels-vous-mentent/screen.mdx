---
title: "Quand vos pixels vous mentent"
description: "Vos écrans ne sont pas aussi parfait que les commerciaux vous le laissent penser ..."
cover: "screen_pixel.png"
---
...
Premièrement, il va être nécessaire de regarder ce qu'est un écran en pratique, et de près !
...
Oui, ce nom a bien disparu depuis quelque temps, mais la technique reste la même derrière.

L'idée : Nous avons une lumière blanche de fond, polarisée (arrivant donc avec le bon angle) sur une
matrice LCD (cristaux liquides) qui va, selon une commande électronique, plus ou moins laisser passer
la lumière vers des filtres colorés (rouges, bleus et verts), notre fameux RGB !
...
Ce n'est pas comme ça qu'on va arriver à avoir un écran rapide nous...

Mais heureusement pour nous, des physiciens ont conçu des méthodes de contrôle qui permettent d'accélérer
ce processus, et réduire, voire supprimer l'erreur. Magique ? Non, pas vraiment. Ces systèmes sont
complexes à mettre en place, et demandent de longs calculs et essais pour arriver à un résultat satisfaisant.
...
Par exemple, il est possible de décaler cette 2ᵉ commande vers une valeur différente, si on voulait
...
commande à fond le temps qu'arrive là où vous voulez, avant de la relâcher vers le niveau souhaité (peut être décalé !).
Pour reprendre l'analogie précédente, c'est comme si après un feu rouge, vous appuyez à fond sur l'accélérateur, 
au lieu d'appuyer juste assez pour arriver à 50 km/h. Vous arriverez nettement plus vite à votre objectif dans le 1er cas !
...
Ces deux problèmes sont typiques des commandes qui appuient à fond. Le système ayant une inertie (si vous ne lâchez
l'accélérateur qu'à 50 km/h, vous dépasserez votre vitesse cible !). Sur un écran, cela se traduit par une couleur
affichée différente de celle voulue.

Je vous ai tracé pour cela un petit graphique :
...
strictement les mêmes sur les graphes, il y a presque un facteur deux en vitesse !).
...
Dans tous les cas, le pixel aura fini son changement avant la période demandée (déterminée par la fréquence de rafraîchissement), 
...
Ce n'est pas une bonne idée non plus, voire pire !
...
Ces gamuts permettent de réduire l'espace couleur absolu en des standard, afin d'assurer qu'une image filmée par un appareil
...
une couleur juste (= strictement identique à celle qui est / sera dans la réalité). À l'opposé, un écran avec une mauvaise colorimétrie
affichera des fausses couleurs (bien souvent trop brillantes / fades).
...
fade en termes de couleur ? Ce n'est pas viable !
...
* 100% NTSC / 100% sRGB : Écrans de graphismes haut de gammes, vous n'êtes probablement pas en train de lire ça ! Idéal pour photo / vidéo !
...
Pour finir, parlons Hz et fréquence de rafraîchissement et ms. Et spoiler : le marketing arrive !
...
* La durée demandée au pixel pour atteindre la 1ʳᵉ fois la couleur, qu'importe de la suite (mesure très courte, même si la durée pour atteindre la couleur sans erreur est 3-5x plus longue !)
...
Changez de vraiment peu la couleur (par exemple d'un Gris à Gris), vous mesurez une réponse très rapide, alors que dans un usage réel, cela sera bien différent !

C'est donc comme ça que votre écran gaming 1 ms est en réalité un écran 15 ms, mesuré de manière différente !
Et ça, le marketing l'a bien compris : ça se vend mieux avec 1 ms.
...
de transition, l'idée n'étant pas de finir de changer de couleur à la fin de l'image). Encore une fois, cette marge ne sera jamais la même
(ex : un écran 1 ms est peut-être un écran 5 ms marge déduite !)

## Conclusion

Et voilà, quelques notions importantes d'un écran vulgarisées.
