---
title: "De texte a processeur"
description: "Dis moi, on conçois comment un processeur ?"
cover: "design_cpu_2.png"
---
...
Vous vous êtes probablement déjà demandé (ou pas), comment on conçoit un processeur
...
Prenons exemple sur les processeurs ”RISC V”, puisqu’ils sont suffisamment simples pour être évoqués ici, 
et en plus, open source, donc, vous aussi pouvez accéder aux [documents](https://riscv.org/specifications/ratified/) ! .
Il faut noter que ces spécifications "ratifiées" sont figées, et ne changeront pas. C'est pour garantir que le
...
Et, ce que l’on peut dire, c’est que ça pique les yeux ! On pourrait croire qu’ils ont découpé les
...
pas un exécutable (.exe) comme habituellement, mais… un schéma électrique ! Et c’est justement là toute la
puissance.

Donc, pour cet article, nous utiliserons un de ces langages, le SystemVerilog, le plus moderne et adapté à
...
précédente, au milieu, la partie calculatoire (elle-même composée de dizaines de blocs !). Et, autour, le bloc
décodeur (qui comprend les instructions !), des compteurs... Des blocs en général plus simples. Le gros de la
logique est au milieu.

Puis, toujours avec le même langage, nous allons les brancher entre eux, etc, jusqu’à finir avec un système
complet. La particularité de ces langages est qu’ils permettent de rester à des niveaux assez élevés (ex : « Je peux
faire A + B>>, pas besoin de faire le circuit logique associé !).

Pour chaque bloc, nous n’oublions pas de le tester : C’est bien beau d’écrire de la logique, mais, il faut être
sûr qu’elle soit parfaite ! Donc, nous y ajoutons aussi un gros noyau de code pour tester le premier. Et, c’est
logique ! Sur du logiciel, vous poussez une mise à jour, et, le problème est réglé. Une fois votre processeur
fabriqué, il sera IMPOSSIBLE de le modifier ! Donc, il faut en être sûr. Dans l’industrie de la conception, 
environ la moitié des équipes sont dédiées aux tests. Pour cela, divers outils de simulation, etc.

Mais, rapidement, ces tests deviendront longs. Très long. Pour donner un ordre d’idée, il n’est pas rare de trouver
...
sont connus : Nvidia, ARM, Intel, AMD. Et, ces appareils vont utiliser des systèmes pour physiquement câbler le
processeur à tester dans leurs circuits, et tester. Ils sont capables de faire démarrer une machine sous Windows
...
:::note
Ce type de machine est capable de faire démarrer Windows en... 20 mn ! Considérant qu'il faudrait plusieurs jours
au meilleur PC que vous pourriez imaginer, ces chiffres sont exceptionnels.
:::

Et, maintenant que nous avons notre code tout mignon, qui marche bien, on en fait quoi ? Ce n'est pas qu’une armoire
...
récents, ils connaissent les failles, et les évitées. [Petite histoire ici](https://en.wikipedia.org/wiki/Pentium_FDIV_bug)
