---
title: "Le-gros-son-de-nos-pc"
description: "Comment et pourquoi le son de pc est-il produit ? Avec quelle qualité ?"
cover: "speaker.png"
---
...
Aujourd'hui, gros articles, assez complexe sur un sujet bien unique : L'audio. Et cette fois-ci, notre cher Deto a
...
Comme un ordinateur ne sait lire que ça, des '0' et des ''1. Comment fait-on ?
...
Un haut-parleur, c'est une membrane mobile, contrôlée par un aimant. Et cet aimant, il est contrôlé par
...
avec un ordre de grandeur important. Un haut-parleur, ça demande de la puissance (qui varie d'un modèle
à un autre), il faut donc beaucoup de courant. À l'opposé, un microphone produit un courant très faible, il
...
cette dernière varie selon la fréquence, elle n'est pas constante, même si les fabricants indiquent une valeur, et
non une fourchette. Par exemple, un haut-parleur n'aura pas la même impédance à 1 kHz ou 10 kHz ! Et ça, c'est
...
permettrait de compenser en partie cet effet, mais ça ne reste pas magique.
...
sortie de ces dernières étant trop limitée, le courant circulant dans le haut-parleur est trop faible et introduit
donc un très petit déplacement, synonyme d'un volume très faible. Dommage ! Vous avez un super casque, et... bah ça ne
marche pas.
...
Ces puces, chef-d'œuvre d'ingénierie électroniques, assurent le gros du travail pour nous. Mais attention, on
...
Ici, l'audio est échantillonné à 192 kHz. Il y a donc 5.208 microsecondes entre chaque point. Cela explique la durée
faible de l'ensemble (environ une milliseconde !). Nous y retrouvons donc la forme du signal audio.
...
plus il y aura d'intervalles, et donc plus qu'ils seront petits. Cela permet de détecter des variations plus fines !

:::note
Au-delà de 22 bits, cela devient inutile : Pour réduire suffisamment le bruit induit par le circuit, vous devriez utiliser
de l'azote liquide !

Bref, les deux derniers bits d'un échantillon 24 bits sont bien présents dans les fichiers, mais que ce soit leur valeur
ou leur représentation physique à l'écoute tient plus des probabilités que de la physique.
:::

La seconde, la vitesse d'enregistrement, qui défini à quel intervalle temporel chaque valeur est mesurée. En général, 
cette valeur varie se situe autour de 44.1 kHz ou 48 kHz, mais peut monter bien plus haut, toujours en multiples de 44.1 kHz et
48 kHz (88.2 kHz ou 96 kHz par exemple) ! Il est possible de trouver, sans grande difficulté des fichiers jusqu’à 192 kHz !

La sélection de cette fréquence cache des mathématiques très complexes, seulement effleurées ici.

L'une des principales se cache dans le repliement spectral, mais késako ? C'est un effet physique complexe, mais fréquemment
...
notre élément), soit 22.050 kHz pour un échantillonnage à 44.1 kHz, 48 kHz pour un échantillonnage à 96 kHz, etc.
...
tant monter en échantillonnage, au-delà de 96 kHz ? À cause d'un effet secondaire, la distorsion harmonique.
...
c'est-à-dire que nous essayons de le "deviner". En général, il est considéré que la valeur n'a pas bougée. Dès lors, excepté aux
moments où nous connaissons la valeur, notre valeur sera fausse. Si nous mesurons suffisamment rapidement, il est possible d'en
...
plus hautes, cela devient critique : Le signal change alors trop vite, et le signal enregistré devient alors... totalement faux !
Et c'est justement ce taux d'erreur qui est mesuré par la distorsion harmonique !

![1khz](./audio_1khz.png)

Nous pouvons ici remarquer que quelle que soit la fréquence d’échantillonnage choisie, le signal réel colle parfaitement à la
...
Et là, patatras ! Seuls les échantillonnages les plus rapides arrivent à reproduire le signal de manière fidèle. Les deux plus
lents (44.1 kHz et 48 kHz) essaient tant bien que mal, mais le résultat ne reproduit pas particulièrement l'entrée !

Si nous accélérions encore notre signal, nous verrions alors le fameux repliement spectral, expliqué plus haut.
...
En électronique, quand on parle de spectre ont fait référence aux différentes fréquences au sein d'un signal (CF : Transformation
de Fourrier). Et, si nous regardons une voix humaine, et bien, on y retrouve la fréquence principale (celle que l'on émet), mais
aussi des harmoniques (cette même fréquence, multipliée par un facteur entier : 2, 4, 6, 8 ...). Et ce sont justement ces harmoniques
...
personnes, chacune avec une voix différente. Ces harmoniques vont sculpter le signal pour le déformer !

Et précisément, la distorsion harmonique ayant une forte influence à "haute" fréquence, elle déforme ces harmoniques, et modifie
donc le timbre de la voix et des différents instruments. Et à ce petit jeu, l'oreille humaine est particulièrement efficace. Un
audiophile entraîné fera sans souci la différence entre deux enregistrements, rien qu'avec cet élément !

A titre d'exemple, voici le spectre audio d'un enregistrement audio :

![spectrum](./audio_spectrum.png)

Ici, il s'agit d'un enregistrement standard. Nous y remarquons une plus grande proportion de basses et moyennes fréquences, là
où les aiguës sont moins présentes. Cela est fait exprès, l'oreille humaine étant plus sensible à ces derniers, ils sont moins
présents dans les enregistrements, afin de conserver une réponse finale standard.

Pour l'anecdote, c'est cette représentation de la donnée qui est utilisée par Shazam et autres outils pour reconnaître une musique
d'une autre. Il agit donc vraiment comme une signature !

À la sortie d'un DAC, nous avons donc un signal analogique, c'est-à-dire qui varie en tension (ou en courant), et non un signal
digital. À l'opposé, quand vous parlez dans un micro par exemple, pour un 'ADC', le fonctionnement est inverse, mais les problèmes
restent les mêmes !
...
de fond dès lors que vous écoutez votre musique avec un système filaire et alimenté par un bloc mural de qualité douteuse. Faites
le  un jour avec un pc portable : Dès qu'il est débranché, ce bruit disparaît.
...
cher, très cher. Et si en plus, vous placez ces éléments dans un milieu défavorable (Par exemple, sur votre carte mère), 
...
alimentations pas conçues pour de l'analogique ! De plus, comme expliqué, c'est quelque chose de complexe et coûteux à développer, 
...
Cette courbe est majoritairement influencée par votre casque, et votre égaliseur, si vous en utilisez un.
...
Il est à noter aussi qu'une catégorie de casque, les casques "gaming" sont particulièrement mauvais à ce jeu. En ayant une conception
douteuse, ils sont souvent incapables de respecter l'audio. Dès lors, dépassé leur usage cible, ils deviennent de mauvais casques.
Notre recommandation est donc choisir un casque adapté à l'ensemble de vos usages, pour ensuite le modifier, via le logiciel
pour peaufiner vos réglages.

## Recommandations

Et pour finir, comme promis : Une petite sélection de produits audios pour tous budgets !
...
<Tabs>
    ...
    <TabItem label="Casques">
        ...
        **Ouverts** (n'apportent aucune isolation passive, mais sont en général plus précis)
        ...
        - Hifiman Edition XS (300€, Planar à plutôt faible sensibilité, modèle de neutralité et d'équilibre dans cette gamme de prix)
        - Meze 105 AER (400€, facile à driver, très dynamique, son vivant et entraînant)
    </TabItem>
